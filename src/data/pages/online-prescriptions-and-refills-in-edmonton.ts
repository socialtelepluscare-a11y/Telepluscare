import type { LandingPageData } from "@/types/landing-page";

export const pageData: LandingPageData = {
  metaTitle: "Online Prescriptions & Refills in Edmonton, Alberta | TelePlusCare",
  metaDescription:
    "Edmonton's same-day virtual prescription service. CPSA-licensed MDs send refills to Whyte Ave, West Ed, Mill Woods and downtown pharmacies. AHCIP covered.",
  title: "Online Prescriptions and Refills in Edmonton",
  heroDescription:
    "Skip the walk-in clinic queue on Jasper Avenue or 124 Street. Edmontonians can connect with a CPSA-licensed virtual MD by video and have a prescription delivered electronically to their preferred pharmacy in West Edmonton, Mill Woods, Whyte Ave, or anywhere across the YEG metro within minutes.",
  heroImage: "/images/image-4.png",
  heroImageAlt: "Online prescriptions and refills in Edmonton Alberta",
  bookingUrl: "/book-appointment-alberta",
  lastReviewed: "2026-04-25",
  reviewedBy: "Dr. Maher Jerudi, MD",
  summary:
    "TelePlusCare offers same-day prescription refill consultations for Edmonton residents through licensed virtual MDs. Scripts are sent electronically to Edmonton-area pharmacies including Shoppers, Rexall, Save-On-Foods, and London Drugs. AHCIP covers physician visits for Albertans with a valid PHN, and no referral is required.",
  keyFacts: [
    "Edmonton has more than 220 community pharmacies across the metro that receive e-prescriptions from TelePlusCare physicians.",
    "Average TelePlusCare appointment for an Edmonton patient takes 12–18 minutes from booking to e-script.",
    "Virtual MD visits are AHCIP-covered for Edmonton residents with a valid Alberta PHN — no referral needed.",
    "Edmonton pharmacies in West Ed, Whyte Ave, Mill Woods, and downtown all accept secure e-prescriptions from CPSA-licensed virtual physicians.",
  ],
  sections: [
    {
      title: "What Edmonton Patients Can Be Prescribed Online",
      content:
        "Edmonton's TelePlusCare virtual physicians prescribe medications across nearly every category of routine outpatient care. Acute prescriptions for UTIs, sinus infections, strep throat, conjunctivitis, and skin infections can typically be issued the same day after a video assessment. Chronic medication renewals — covering everything most Edmontonians take long-term — are handled with a quick medication review. Complex initiations like new antidepressants or new diabetes therapies are done after a more thorough first-visit history.",
      bullets: [
        "Antibiotics for UTI, sinusitis, strep, skin infections",
        "Blood pressure medications (Ramipril, Amlodipine, Telmisartan)",
        "Cholesterol statins (Atorvastatin, Rosuvastatin)",
        "Type 2 diabetes — Metformin, Jardiance, Trulicity, Ozempic",
        "Asthma controllers and inhalers (Symbicort, Advair, Ventolin)",
        "Birth control pills, patches, and rings",
        "SSRIs/SNRIs for depression and anxiety (non-controlled)",
        "Thyroid hormone replacement (Synthroid)",
      ],
      image: "/images/Rectangle-1090-3.png",
      imageAlt: "Medications prescribed online for Edmonton residents",
    },
    {
      title: "Common Prescription Refills for Edmonton Residents",
      content:
        "Edmonton's mix of urban professionals, university students at U of A and MacEwan, families in Sherwood Park-adjacent suburbs, and shift workers at the Edmonton International Airport drives consistent demand for fast prescription renewals. The most common requests our Edmonton patients book are blood pressure refills, statin renewals, oral contraceptive renewals (especially Alesse, Yasmin, and Mirvala), Symbicort and Ventolin for asthma, Cipralex and Wellbutrin for mood, and Synthroid for thyroid maintenance. Many Edmontonians stack two to three medications into a single 15-minute virtual visit.",
      bullets: [
        "High blood pressure renewals (ACE/ARB/CCB classes)",
        "Cholesterol-lowering statin refills",
        "Birth control pill renewals",
        "Asthma inhaler refills",
        "Antidepressant maintenance",
        "Thyroid medication renewals",
      ],
      image: "/images/Rectangle-1090-6.png",
      imageAlt: "Common refills for Edmonton patients",
    },
    {
      title: "How Edmonton Online Prescriptions Work",
      content:
        "Booking with TelePlusCare is built around three steps. Step 1 — Choose your appointment slot online and enter your Alberta PHN, the medication(s) you need, and your preferred Edmonton pharmacy. Step 2 — Connect by secure video with a CPSA-registered family physician; the visit averages 12–18 minutes. Step 3 — Once approved, your prescription is electronically transmitted to the pharmacy you selected — whether that's the Shoppers at Southgate, the Rexall on Jasper Avenue, the Save-On-Foods Pharmacy in Windermere, or your local independent.",
      bullets: [
        "Step 1 — Book online with your Alberta PHN",
        "Step 2 — 12–18 minute video visit with an Edmonton-licensed MD",
        "Step 3 — E-prescription sent to your chosen Edmonton pharmacy",
      ],
      image: "/images/prescription-refill/111-819x1024.jpeg",
      imageAlt: "Three-step Edmonton prescription process",
    },
    {
      title: "Prescription Delivered to Your Local Edmonton Pharmacy",
      content:
        "TelePlusCare prescriptions can go to any of the 220+ pharmacies serving Edmonton and surrounding suburbs. Downtown patients often choose the Shoppers Drug Mart at 102 Avenue, the Rexall on Jasper Avenue, or London Drugs at City Centre. West Edmonton residents typically use the West Edmonton Mall pharmacies, the Save-On-Foods at Lewis Estates, or Pharmasave on Stony Plain Road. Mill Woods, Whyte Ave (Old Strathcona), Sherwood Park, and Riverbend patients all have multiple options. Same-day home delivery is widely available across the metro through Shoppers, Pharmasave, and several independents.",
      bullets: [
        "Shoppers Drug Mart — Southgate, Jasper Ave, Mill Woods, West End",
        "Rexall — downtown, Whyte Avenue, North Edmonton",
        "London Drugs — City Centre, West Edmonton Mall, Capilano",
        "Save-On-Foods Pharmacy — Windermere, Lewis Estates, Riverbend",
        "Sobeys, Loblaws, Pharmasave Edmonton-wide",
      ],
      image: "/images/Rectangle-1090-20.png",
      imageAlt: "Edmonton pharmacies receiving e-prescriptions",
    },
    {
      title: "Local Pharmacy Partners Across Edmonton",
      content:
        "We work directly with all major chains and dozens of independent pharmacies serving distinct Edmonton neighbourhoods. North Edmonton residents often use Castle Downs and Northgate locations. Patients in Mill Woods and the south side rely on Mill Woods Town Centre and South Edmonton Common pharmacies. University students in Garneau use the Whyte Avenue locations. Albertans visiting NAIT or working at the Royal Alexandra Hospital area have several nearby options on Kingsway. We also send prescriptions to St. Albert, Sherwood Park, Beaumont, and Leduc pharmacies for greater Edmonton commuters.",
      bullets: [
        "Castle Downs, Northgate, Clareview pharmacies (north)",
        "Mill Woods Town Centre, South Edmonton Common (south)",
        "Whyte Avenue, Garneau (university area)",
        "Kingsway, Westmount (central)",
        "St. Albert, Sherwood Park, Beaumont (commuter belt)",
      ],
      image: "/images/Rectangle-1090-22.png",
      imageAlt: "Local Edmonton pharmacy partners",
    },
    {
      title: "Why Edmonton Patients Choose TelePlusCare",
      content:
        "Edmonton's healthcare landscape has long wait times at walk-in clinics and family practices. TelePlusCare cuts the wait by offering same-day virtual appointments seven days a week, evenings included. Patients booking before noon almost always have their prescription at their pharmacy by mid-afternoon. We're trusted by Edmonton families, U of A students, oilfield workers commuting north, and busy professionals downtown.",
      bullets: [
        "Same-day appointments — most slots within 2 hours",
        "Evenings and weekends available",
        "AHCIP-billed for Edmonton residents",
        "100% CPSA-registered Alberta physicians",
      ],
      image: "/images/image-5.png",
      imageAlt: "Why Edmonton patients choose TelePlusCare",
    },
    {
      title: "Alberta Health Prescription Coverage for Edmonton",
      content:
        "Edmontonians benefit from the full suite of Alberta drug coverage programs. AHCIP pays for the virtual physician consultation. Medication coverage flows through Alberta Blue Cross Non-Group, Coverage for Seniors (auto-enrolled at 65), the Adult Health Benefit for low-income Edmontonians, and Palliative Coverage. Many University of Alberta and MacEwan students have additional coverage through their student health plan. Most Edmonton employers — including the City of Edmonton, Stantec, ATB Financial, and Capital Power — offer extended health benefits.",
      bullets: [
        "AHCIP covers physician consult fee",
        "Alberta Blue Cross Non-Group plan",
        "Coverage for Seniors (65+)",
        "Adult Health Benefit (income-based)",
      ],
      image: "/images/image-6.png",
      imageAlt: "Edmonton Alberta Health drug coverage options",
    },
  ],
  faqs: [
    {
      question: "How fast can I get a prescription in Edmonton through TelePlusCare?",
      answer:
        "Most Edmonton patients receive their prescription at their pharmacy within 1–2 hours of booking, often sooner during weekday clinic hours.",
    },
    {
      question: "Which Edmonton pharmacies accept TelePlusCare e-prescriptions?",
      answer:
        "Every licensed pharmacy in Edmonton accepts our e-prescriptions, including Shoppers Drug Mart, Rexall, London Drugs, Save-On-Foods, Sobeys, Loblaws, Pharmasave, and all independents.",
    },
    {
      question: "Is the virtual visit covered by Alberta Health for Edmonton residents?",
      answer:
        "Yes. If you have a valid Alberta PHN, AHCIP covers the physician's virtual consultation fee. The medication itself is billed by your pharmacy under your drug plan.",
    },
    {
      question: "Can I get my Symbicort or Ventolin inhaler refilled online in Edmonton?",
      answer:
        "Yes. Asthma controller and rescue inhalers are among the most commonly refilled medications by our Edmonton patients. A brief virtual review is all that's typically needed.",
    },
    {
      question: "Do I need a family doctor in Edmonton to use TelePlusCare?",
      answer:
        "No. You don't need an existing family doctor. Any Edmonton resident with an Alberta PHN can book directly with one of our CPSA-licensed virtual MDs.",
    },
  ],
  locations:
    "Serving downtown Edmonton, Whyte Avenue, Old Strathcona, West Edmonton, Mill Woods, Riverbend, Windermere, Castle Downs, Clareview, Sherwood Park, St. Albert, Beaumont, Leduc, Spruce Grove, Stony Plain, and the entire Edmonton metropolitan region.",
};
